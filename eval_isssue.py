...
print("")
X,X_train,X_test,dummy_y_train,dummy_y_test,num_classes,encoder = load_data_issue.load_data_issue_and_labels()
...
if FLAGS.eval_train:
    ...
    df = pandas.read_csv("./issueTestUnique.csv")
    df = pandas.DataFrame(df)
    x_raw = df['Consumer complaint narrative'].astype('str')
    y_encoded_test = list(df['Issue'].astype('str'))
...
predicted = open("./predicted_issue.csv","w")
for values in all_predictions:
	predicted.write(str(values))
	predicted.write("\n")
predicted.close()
